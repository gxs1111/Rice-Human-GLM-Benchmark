...
class TrainingArguments(transformers.TrainingArguments):
    ...
    run_name: str = field(default="DNABERT2_region")
...
def split_dataset(data_path: str, output_dir: str, train_ratio=0.8, val_ratio=0.1):
    ...
    region_to_id = {"exon": 0, "intron": 1, "intergenic": 2}
    numeric_labels = [region_to_id[label] for label in labels]
    ...
    train_data = [[sequences[train_val_idx[i]], labels[train_val_idx[i]]] for i in train_idx]
    val_data = [[sequences[train_val_idx[i]], labels[train_val_idx[i]]] for i in val_idx]
...
class SupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, kmer: int = -1):
        ...
        region_to_id = {"exon": 0, "intron": 1, "intergenic": 2}
        labels = [region_to_id[d[1]] for d in data]
...
def calculate_metric_with_sklearn(predictions: np.ndarray, labels: np.ndarray):
    ...
    region_names = ["exon", "intron", "intergenic"]
    cm_dict = {"confusion_matrix": cm.tolist(), "labels": region_names}
