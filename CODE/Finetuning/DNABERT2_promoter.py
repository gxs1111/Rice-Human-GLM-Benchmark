...
class TrainingArguments(transformers.TrainingArguments):
    ...
    run_name: str = field(default="DNABERT2_promoter")
    optim: str = field(default="adamw_torch")
    model_max_length: int = field(default=MAX_LENGTH, metadata={"help": "Maximum sequence length."})
...
def get_reverse_complement(sequence: str):
    MAP = {"A": "T", "T": "A", "C": "G", "G": "C"}
    return "".join([MAP[c] for c in sequence[::-1]])
    ...
    def __init__(self,
        ...
        for text in texts:
            if isinstance(text, list):
                token_length = len(tokenizer(text[0], add_special_tokens=True)["input_ids"]) + \
                               len(tokenizer(text[1], add_special_tokens=True)["input_ids"])
            else:
                token_length = len(tokenizer(text, add_special_tokens=True)["input_ids"])
            max_token_length = max(max_token_length, token_length)
        print(f"Maximum token length in dataset {data_path}: {max_token_length}")
        output = tokenizer(
            texts,
            return_tensors="pt",
            padding="longest",
...
def calculate_metric_with_sklearn(predictions: np.ndarray, labels: np.ndarray):
    ...
    return {
        ...
        "f1": sklearn.metrics.f1_score(valid_labels, valid_predictions, average="binary", pos_label=1, zero_division=0),
        "matthews_correlation": sklearn.metrics.matthews_corrcoef(valid_labels, valid_predictions),
        "precision": sklearn.metrics.precision_score(valid_labels, valid_predictions, average="binary", pos_label=1, zero_division=0),
        "recall": sklearn.metrics.recall_score(valid_labels, valid_predictions, average="binary", pos_label=1, zero_division=0),
    }
