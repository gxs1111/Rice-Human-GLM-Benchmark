...
NONPROMOTER_FASTA = "<NONPROMOTER_FASTA>"
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def __init__(self, config, use_head=True, n_classes=2):
    ...
    @classmethod
    def from_pretrained(cls, weights_path, config, device='cpu', use_head=True, n_classes=2):
...
promoter_data = load_fasta(PROMOTER_FASTA, label=1)
nonpromoter_data = load_fasta(NONPROMOTER_FASTA, label=0)
all_data = promoter_data + nonpromoter_data
...
val_seq, test_seq, val_labels, test_labels = train_test_split(
    temp_seq, temp_labels, test_size=0.5, stratify=temp_labels, random_state=42
)
...
train_dataset = PromoterDataset(train_seq, train_labels, tokenizer, MAX_LENGTH)
val_dataset = PromoterDataset(val_seq, val_labels, tokenizer, MAX_LENGTH)
test_dataset = PromoterDataset(test_seq, test_labels, tokenizer, MAX_LENGTH)
