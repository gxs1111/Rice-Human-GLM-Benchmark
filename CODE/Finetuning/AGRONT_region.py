...
sequences, labels, lengths = read_csv(CONFIG["data_file"])
...
for lbl in set(labels):
    ...
    balanced_labels.extend([lbl] * len(selected_indices))

unique_labels = sorted(set(balanced_labels))
label_to_id = {lbl: idx for idx, lbl in enumerate(unique_labels)}
encoded_labels = [label_to_id[lbl] for lbl in balanced_labels]
print(f"Label mapping: {label_to_id}")

data = {"sequence": balanced_sequences, "labels": encoded_labels}
...
plt.savefig(CONFIG["confusion_matrix_file"], dpi=300, bbox_inches='tight')
plt.close()
