...
positive_tss_fasta = "<POSITIVE_TSS_FASTA>"
negative_tss_fasta = "<NEGATIVE_TSS_FASTA>"
...
print("Loading TSS positive and negative sequences...")
positive_tss_seqs, positive_tss_labels = load_fasta(positive_tss_fasta, 1)
negative_tss_seqs, negative_tss_labels = load_fasta(negative_tss_fasta, 0)

sequences = positive_tss_seqs + negative_tss_seqs
...
print(f"Total dataset: Positive (TSS) = {len(positive_data)}, Negative (non-TSS) = {len(negative_data)}")
...
for split_name, split_data in [("Train", train_data), ("Validation", val_data), ("Test", test_data)]:
    ...
    print(f"{split_name}: Positive (TSS) = {pos_count}, Negative (non-TSS) = {neg_count}")
...
for i in range(min(3, len(data))):
    ...
    print(f"Label: {label} (1=TSS, 0=non-TSS)")
