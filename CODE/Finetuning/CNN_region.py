...
def prepare_data(data_file):
    ...
    val_dataset = RegionDataset(X_val, y_val)
...
def train_model(model, train_loader, val_loader, device, epochs=EPOCHS, patience=PATIENCE):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1:2d}/{epochs} | Loss: {running_loss/len(train_loader):.4f} | "
              f"Val Acc: {val_acc:.4f} | Macro Precision: {val_precision:.4f} | "
...
def evaluate_model(model, test_loader, device, le):
    ...
    print(f"MCC:          {mcc:.4f}")
    print(f"Confusion Matrix (order: {list(le.classes_)}):")
    print(cm)
    ...
    result_str = (f"Test Accuracy: {acc:.4f}\n"
                  ...
                  f"Macro F1: {f1:.4f}\n"
                  f"MCC: {mcc:.4f}\n"
                  f"Label order: {list(le.classes_)}\n"
                  f"Confusion Matrix:\n{cm}\n")
...
def main():
    ...
    model = RegionCNN(num_classes).to(device)
